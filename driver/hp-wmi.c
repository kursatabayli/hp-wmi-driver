...
static int hp_wmi_fan_speed_set(struct hp_wmi_hwmon_priv *priv)
{
	...
	fan_speed[CPU_FAN] = pwm_to_rpm(priv->pwm_cpu, priv);
	fan_speed[GPU_FAN] = pwm_to_rpm(priv->pwm_gpu, priv);
...
{
	priv->pwm_cpu = HP_FAN_SPEED_AUTOMATIC;
	priv->pwm_gpu = HP_FAN_SPEED_AUTOMATIC;
			      ...
			      u32 attr, int channel, long val)
{
	...
	case hwmon_pwm:
		if (attr == hwmon_pwm_input) {
			...
			if (channel == 0)
				priv->pwm_cpu = rpm_to_pwm(rpm, priv);
			else
				priv->pwm_gpu = rpm_to_pwm(rpm, priv);
		...
		case PWM_MODE_MANUAL:
			...
			if (rpm >= 0)
				priv->pwm_cpu = rpm_to_pwm(rpm / 100, priv);
			rpm = hp_wmi_get_fan_speed_victus_s(GPU_FAN);
			if (rpm >= 0)
				priv->pwm_gpu = rpm_to_pwm(rpm / 100, priv);
